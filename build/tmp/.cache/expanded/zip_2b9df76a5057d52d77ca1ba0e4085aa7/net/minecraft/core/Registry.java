...
public interface Registry<T> extends Keyable, IdMap<T> {
    ...
    default Codec<T> byNameCodec() {
        return this.m_322937_().flatComapMap(Holder.Reference::value, p_325680_ -> this.m_324863_(this.wrapAsHolder((T)p_325680_)));
    }

    default Codec<Holder<T>> holderByNameCodec() {
        return this.m_322937_().flatComapMap(p_325683_ -> (Holder<T>)p_325683_, this::m_324863_);
    }

    private Codec<Holder.Reference<T>> m_322937_() {
        Codec<Holder.Reference<T>> codec = ResourceLocation.CODEC
            .comapFlatMap(
                p_325684_ -> this.m_320017_(p_325684_)
                        .map(DataResult::success)
                        .orElseGet(() -> DataResult.error(() -> "Unknown registry key in " + this.key() + ": " + p_325684_)),
                p_325675_ -> p_325675_.key().location()
            );
        return ExtraCodecs.m_322767_(codec, p_325682_ -> this.lifecycle(p_325682_.key()).map(RegistrationInfo::f_313951_).orElse(Lifecycle.experimental()));
    }

    private DataResult<Holder.Reference<T>> m_324863_(Holder<T> p_329506_) {
    ...
    Optional<RegistrationInfo> lifecycle(ResourceKey<T> p_333179_);
    ...
    static <V, T extends V> T register(Registry<V> pRegistry, ResourceKey<V> pKey, T pValue) {
        ((WritableRegistry)pRegistry).register(pKey, (V)pValue, RegistrationInfo.f_316022_);
        return pValue;
    }

    static <T> Holder.Reference<T> registerForHolder(Registry<T> pRegistry, ResourceKey<T> pKey, T pValue) {
        return ((WritableRegistry)pRegistry).register(pKey, pValue, RegistrationInfo.f_316022_);
    }
    ...
    Optional<Holder.Reference<T>> m_320017_(ResourceLocation p_329586_);
    ...
    default HolderLookup.RegistryLookup<T> asTagAddingLookup() {
        return new HolderLookup.RegistryLookup.Delegate<T>() {
            ...
            @Override
            public Optional<HolderSet.Named<T>> m_255050_(TagKey<T> p_259111_) {

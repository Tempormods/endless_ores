...
public class BannerPatternFormatFix extends NamedEntityFix {
    private static final Map<String, String> f_314037_ = Map.ofEntries(
    ...
    protected Typed<?> fix(Typed<?> p_332978_) {
        return p_332978_.update(DSL.remainderFinder(), BannerPatternFormatFix::m_318907_);
    }

    private static Dynamic<?> m_318907_(Dynamic<?> p_329398_) {
        return p_329398_.renameAndFixField(
            "Patterns", "patterns", p_330184_ -> p_330184_.createList(p_330184_.asStream().map(BannerPatternFormatFix::m_321308_))
        );
    }

    private static Dynamic<?> m_321308_(Dynamic<?> p_333413_) {
        p_333413_ = p_333413_.renameAndFixField(
            "Pattern",
            "pattern",
            p_328292_ -> DataFixUtils.orElse(
                    p_328292_.asString().map(p_331883_ -> f_314037_.getOrDefault(p_331883_, p_331883_)).map(p_328292_::createString).result(), p_328292_
                )
        );
        return p_333413_.renameAndFixField("Color", "color", p_335083_ -> p_335083_.createString(m_322288_(p_335083_.asInt(0))));
    }

    public static String m_322288_(int p_328642_) {

...
public class ParticleUnflatteningFix extends DataFix {
    private static final Logger f_316319_ = LogUtils.getLogger();
    ...
    protected TypeRewriteRule makeRule() {
        Type<?> type = this.getInputSchema().getType(References.f_315042_);
        Type<?> type1 = this.getOutputSchema().getType(References.f_315042_);
        return this.writeFixAndRead("ParticleUnflatteningFix", type, type1, this::m_323954_);
    }

    private <T> Dynamic<T> m_323954_(Dynamic<T> p_329773_) {
        ...
        if (optional.isEmpty()) {
            ...
            return switch (s1) {
                case "minecraft:item" -> astring.length > 1 ? this.m_318610_(dynamic, astring[1]) : dynamic;
                case "minecraft:block", "minecraft:block_marker", "minecraft:falling_dust", "minecraft:dust_pillar" -> astring.length > 1
                ? this.m_324092_(dynamic, astring[1])
                : dynamic;
                case "minecraft:dust" -> astring.length > 1 ? this.m_318891_(dynamic, astring[1]) : dynamic;
                case "minecraft:dust_color_transition" -> astring.length > 1 ? this.m_321417_(dynamic, astring[1]) : dynamic;
                case "minecraft:sculk_charge" -> astring.length > 1 ? this.m_320841_(dynamic, astring[1]) : dynamic;
                case "minecraft:vibration" -> astring.length > 1 ? this.m_321826_(dynamic, astring[1]) : dynamic;
                case "minecraft:shriek" -> astring.length > 1 ? this.m_318673_(dynamic, astring[1]) : dynamic;
                default -> dynamic;
            };
        }
    }

    private <T> Dynamic<T> m_318610_(Dynamic<T> p_336140_, String p_328811_) {
        ...
        if (i == -1) {
            ...
            CompoundTag compoundtag = m_322229_(p_328811_.substring(i));
            if (compoundtag != null) {
    ...
    @Nullable
    private static CompoundTag m_322229_(String p_328375_) {
        try {
            return TagParser.parseTag(p_328375_);
        } catch (Exception exception) {
            f_316319_.warn("Failed to parse tag: {}", p_328375_, exception);
            return null;
        }
    }

    private <T> Dynamic<T> m_324092_(Dynamic<T> p_330510_, String p_334026_) {
        ...
        if (i == -1) {
            ...
            Map<Dynamic<T>, Dynamic<T>> map = m_324864_(p_330510_, p_334026_.substring(i));
            if (!map.isEmpty()) {
    ...
    private static <T> Map<Dynamic<T>, Dynamic<T>> m_324864_(Dynamic<T> p_328751_, String p_332614_) {
        try {
            ...
            f_316319_.warn("Failed to parse block properties: {}", p_332614_, exception);
            return Map.of();
        }
    }

    private static <T> Dynamic<T> m_324339_(Dynamic<T> p_333163_, StringReader p_332387_) throws CommandSyntaxException {
    ...
    private <T> Dynamic<T> m_318891_(Dynamic<T> p_335964_, String p_335720_) {
        try {
            StringReader stringreader = new StringReader(p_335720_);
            Dynamic<T> dynamic = m_324339_(p_335964_, stringreader);
            stringreader.expect(' ');
            float f = stringreader.readFloat();
            return p_335964_.set("color", dynamic).set("scale", p_335964_.createFloat(f));
        } catch (Exception exception) {
            f_316319_.warn("Failed to parse particle options: {}", p_335720_, exception);
            return p_335964_;
        }
    }

    private <T> Dynamic<T> m_321417_(Dynamic<T> p_328807_, String p_329378_) {
        try {
            StringReader stringreader = new StringReader(p_329378_);
            Dynamic<T> dynamic = m_324339_(p_328807_, stringreader);
            stringreader.expect(' ');
            float f = stringreader.readFloat();
            stringreader.expect(' ');
            Dynamic<T> dynamic1 = m_324339_(p_328807_, stringreader);
            return p_328807_.set("from_color", dynamic).set("to_color", dynamic1).set("scale", p_328807_.createFloat(f));
        } catch (Exception exception) {
            f_316319_.warn("Failed to parse particle options: {}", p_329378_, exception);
            return p_328807_;
        }
    }

    private <T> Dynamic<T> m_320841_(Dynamic<T> p_335265_, String p_329894_) {
        try {
            StringReader stringreader = new StringReader(p_329894_);
            float f = stringreader.readFloat();
            return p_335265_.set("roll", p_335265_.createFloat(f));
        } catch (Exception exception) {
            f_316319_.warn("Failed to parse particle options: {}", p_329894_, exception);
            return p_335265_;
        }
    }

    private <T> Dynamic<T> m_321826_(Dynamic<T> p_333915_, String p_333536_) {
        try {
            ...
            f_316319_.warn("Failed to parse particle options: {}", p_333536_, exception);
            return p_333915_;
        }
    }

    private <T> Dynamic<T> m_318673_(Dynamic<T> p_330969_, String p_330514_) {
        try {
            StringReader stringreader = new StringReader(p_330514_);
            int i = stringreader.readInt();
            return p_330969_.set("delay", p_330969_.createInt(i));
        } catch (Exception exception) {
            f_316319_.warn("Failed to parse particle options: {}", p_330514_, exception);
            return p_330969_;
        }
    }
}
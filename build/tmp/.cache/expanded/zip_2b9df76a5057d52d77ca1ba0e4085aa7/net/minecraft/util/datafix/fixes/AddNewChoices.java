...
public class AddNewChoices extends DataFix {
    ...
    private <K> TypeRewriteRule cap(TaggedChoiceType<K> pType, TaggedChoiceType<?> pNewType) {
        if (pType.getKeyType() != pNewType.getKeyType()) {
            ...
            return this.fixTypeEverywhere(
                this.name,
                pType,
                (TaggedChoiceType<K>)pNewType,
                p_14636_ -> p_326542_ -> {
                        if (!((TaggedChoiceType<K>)pNewType).hasType(p_326542_.getFirst())) {

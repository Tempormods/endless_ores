...
public class ItemStackCustomNameToOverrideComponentFix extends DataFix {
    private static final Set<String> f_316114_ = Set.of(
            ...
            p_328527_ -> {
                ...
                if (optional1.filter(p_329654_ -> p_329654_.equals("minecraft:white_banner")).isPresent()) {
                    return p_328527_.updateTyped(
                        opticfinder1, p_331207_ -> p_331207_.update(DSL.remainderFinder(), ItemStackCustomNameToOverrideComponentFix::m_319125_)
                    );
                } else {
                    return optional1.filter(p_336047_ -> p_336047_.equals("minecraft:filled_map")).isPresent()
                        ? p_328527_.updateTyped(
                            opticfinder1, p_332534_ -> p_332534_.update(DSL.remainderFinder(), ItemStackCustomNameToOverrideComponentFix::m_324602_)
                        )
                        : p_328527_;
                }
            }
    ...
    private static <T> Dynamic<T> m_324602_(Dynamic<T> p_331719_) {
        return m_324969_(p_331719_, f_316114_::contains);
    }

    private static <T> Dynamic<T> m_319125_(Dynamic<T> p_328946_) {
        return m_324969_(p_328946_, p_329571_ -> p_329571_.equals("block.minecraft.ominous_banner"));
    }

    private static <T> Dynamic<T> m_324969_(Dynamic<T> p_335170_, Predicate<String> p_334171_) {
        OptionalDynamic<T> optionaldynamic = p_335170_.get("minecraft:custom_name");
        Optional<String> optional = optionaldynamic.asString().result().flatMap(ComponentDataFixUtils::m_321143_).filter(p_334171_);

...
public class AdvancementCommands {
    private static final DynamicCommandExceptionType f_302468_ = new DynamicCommandExceptionType(p_308608_ -> (Component)p_308608_);
    private static final Dynamic2CommandExceptionType f_302376_ = new Dynamic2CommandExceptionType(
    ...
    private static int perform(
        ...
        if (i == 0) {
            if (pAdvancements.size() == 1) {
                ...
                throw f_302468_.create(Component.translatable(pAction.getKey() + ".many.to.many.failure", pAdvancements.size(), pTargets.size()));
            }
        } else {
            if (pAdvancements.size() == 1) {
                if (pTargets.size() == 1) {
    ...
    private static int performCriterion(
        ...
        if (!advancement.criteria().containsKey(pCriterionName)) {
            throw f_302376_.create(Advancement.name(pAdvancement), pCriterionName);
        } else {
            for (ServerPlayer serverplayer : pTargets) {

...
public class PackRepository {
    ...
    public static String m_319340_(Collection<Pack> p_331712_) {
    ...
    private List<Pack> rebuildSelected(Collection<String> pIds) {
        List<Pack> list = this.getAvailablePacks(pIds).collect(Util.m_323807_());

        for (Pack pack : this.available.values()) {
            if (pack.isRequired() && !list.contains(pack)) {
                pack.getDefaultPosition().insert(list, pack, Pack::m_319332_, false);
            }
        }

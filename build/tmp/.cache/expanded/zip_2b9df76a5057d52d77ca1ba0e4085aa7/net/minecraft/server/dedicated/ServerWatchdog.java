...
public class ServerWatchdog implements Runnable {
    ...
    private final long f_302422_;

    public ServerWatchdog(DedicatedServer pServer) {
        this.server = pServer;
        this.f_302422_ = pServer.getMaxTickLength() * TimeUtil.NANOSECONDS_PER_MILLISECOND;
    }
        ...
        while (this.server.isRunning()) {
            ...
            if (k > this.f_302422_) {
                LOGGER.error(
                    ...
                    String.format(Locale.ROOT, "%.2f", this.server.m_306290_().m_305111_() / (float)TimeUtil.f_302812_)
                );
            ...
            try {
                Thread.sleep((i + this.f_302422_ - j) / TimeUtil.NANOSECONDS_PER_MILLISECOND);

...
public class EntityRenderDispatcher implements ResourceManagerReloadListener {
    ...
    public <E extends Entity> void render(
        ...
        try {
            ...
            if (pEntity.displayFireAnimation()) {
                this.renderFlame(pPoseStack, pBuffer, pEntity, Mth.m_305706_(Mth.f_303648_, this.cameraOrientation, new Quaternionf()));
            }

            pPoseStack.translate(-vec3.x(), -vec3.y(), -vec3.z());
            if (this.options.entityShadows().get() && this.shouldRenderShadow && !pEntity.isInvisible()) {
                float f = entityrenderer.m_318622_(pEntity);
                if (f > 0.0F) {
    ...
    private static void renderHitbox(PoseStack pPoseStack, VertexConsumer pBuffer, Entity pEntity, float pPartialTicks) {
        ...
        pBuffer.m_320578_(posestack$pose, 0.0F, pEntity.getEyeHeight(), 0.0F)
            .color(0, 0, 255, 255)
    ...
    private void renderFlame(PoseStack pPoseStack, MultiBufferSource pBuffer, Entity pEntity, Quaternionf p_312342_) {
    ...
    private static void fireVertex(
        ...
        pBuffer.m_320578_(pMatrixEntry, pX, pY, pZ)

...
public class DecoratedPotRenderer implements BlockEntityRenderer<DecoratedPotBlockEntity> {
    ...
    private static final float f_303154_ = 0.125F;
    ...
    public void render(DecoratedPotBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        ...
        pPoseStack.translate(-0.5, 0.0, -0.5);
        DecoratedPotBlockEntity.WobbleStyle decoratedpotblockentity$wobblestyle = pBlockEntity.f_302709_;
        if (decoratedpotblockentity$wobblestyle != null && pBlockEntity.getLevel() != null) {
            float f = ((float)(pBlockEntity.getLevel().getGameTime() - pBlockEntity.f_303253_) + pPartialTick) / (float)decoratedpotblockentity$wobblestyle.f_303394_;
            if (f >= 0.0F && f <= 1.0F) {
        ...
        this.renderSide(this.frontSide, pPoseStack, pBuffer, pPackedLight, pPackedOverlay, getMaterial(potdecorations.f_314229_()));
        this.renderSide(this.backSide, pPoseStack, pBuffer, pPackedLight, pPackedOverlay, getMaterial(potdecorations.f_316149_()));
        this.renderSide(this.leftSide, pPoseStack, pBuffer, pPackedLight, pPackedOverlay, getMaterial(potdecorations.f_315094_()));
        this.renderSide(this.rightSide, pPoseStack, pBuffer, pPackedLight, pPackedOverlay, getMaterial(potdecorations.f_315756_()));
        pPoseStack.popPose();
    }

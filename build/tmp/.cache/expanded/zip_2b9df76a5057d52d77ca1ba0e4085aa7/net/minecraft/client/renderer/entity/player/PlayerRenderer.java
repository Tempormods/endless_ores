...
public class PlayerRenderer extends LivingEntityRenderer<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>> {
    ...
    protected void renderNameTag(
        AbstractClientPlayer pEntity, Component pDisplayName, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, float p_336395_
    ) {
        ...
        if (d0 < 100.0) {
            ...
            if (objective != null) {
                ReadOnlyScoreInfo readonlyscoreinfo = scoreboard.m_305759_(pEntity, objective);
                Component component = ReadOnlyScoreInfo.m_305849_(readonlyscoreinfo, objective.m_305063_(StyledFormat.f_302669_));
                super.renderNameTag(
                    pEntity,
        ...
        super.renderNameTag(pEntity, pDisplayName, pPoseStack, pBuffer, pPackedLight, p_336395_);
        pPoseStack.popPose();
    }
    ...
    protected void setupRotations(AbstractClientPlayer pEntityLiving, PoseStack pPoseStack, float pAgeInTicks, float pRotationYaw, float pPartialTicks, float p_335509_) {
        float f = pEntityLiving.getSwimAmount(pPartialTicks);
        float f1 = pEntityLiving.getViewXRot(pPartialTicks);
        if (pEntityLiving.isFallFlying()) {
            super.setupRotations(pEntityLiving, pPoseStack, pAgeInTicks, pRotationYaw, pPartialTicks, p_335509_);
            ...
            super.setupRotations(pEntityLiving, pPoseStack, pAgeInTicks, pRotationYaw, pPartialTicks, p_335509_);
            float f4 = pEntityLiving.isInWater() || pEntityLiving.isInFluidType((fluidType, height) -> pEntityLiving.canSwimInFluidType(fluidType)) ? -90.0F - f1 : -90.0F;

...
public class DrownedRenderer extends AbstractZombieRenderer<Drowned, DrownedModel<Drowned>> {
    ...
    protected void setupRotations(Drowned pEntityLiving, PoseStack pPoseStack, float pAgeInTicks, float pRotationYaw, float pPartialTicks, float p_334653_) {
        super.setupRotations(pEntityLiving, pPoseStack, pAgeInTicks, pRotationYaw, pPartialTicks, p_334653_);
        float f = pEntityLiving.getSwimAmount(pPartialTicks);
        if (f > 0.0F) {
            float f1 = -10.0F - pEntityLiving.getXRot();
            float f2 = Mth.lerp(f, 0.0F, f1);
            pPoseStack.rotateAround(Axis.XP.rotationDegrees(f2), 0.0F, pEntityLiving.getBbHeight() / 2.0F / p_334653_, 0.0F);
        }
    }
}
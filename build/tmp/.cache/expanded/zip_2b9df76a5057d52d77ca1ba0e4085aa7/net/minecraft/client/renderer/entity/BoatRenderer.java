...
public class BoatRenderer extends EntityRenderer<Boat> {
    ...
    public void render(Boat pEntity, float pEntityYaw, float pPartialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        float f = (float)pEntity.m_305464_() - pPartialTicks;
        float f1 = pEntity.m_304923_() - pPartialTicks;
        if (f1 < 0.0F) {
            f1 = 0.0F;
        }

        if (f > 0.0F) {
            pPoseStack.mulPose(Axis.XP.rotationDegrees(Mth.sin(f) * f * f1 / 10.0F * (float)pEntity.m_305195_()));
        }

...
public class EditBox extends AbstractWidget implements Renderable {
    ...
    public void insertText(String pTextToWrite) {
        ...
        if (k > 0) {
            String s = StringUtil.m_319203_(pTextToWrite);
            int l = s.length();
            if (k < l) {
    ...
    public void deleteWords(int pNum) {
        if (!this.value.isEmpty()) {
            if (this.highlightPos != this.cursorPos) {
                ...
                this.m_306194_(this.getWordPosition(pNum));
            }
        }
    }

    public void deleteChars(int pNum) {
        this.m_306194_(this.getCursorPos(pNum));
    }

    public void m_306194_(int p_310763_) {
        if (!this.value.isEmpty()) {
            if (this.highlightPos != this.cursorPos) {
                this.insertText("");
            } else {

...
public class CartographyTableScreen extends AbstractContainerScreen<CartographyTableMenu> {
    ...
    protected void renderBg(GuiGraphics pGuiGraphics, float pPartialTick, int pMouseX, int pMouseY) {
        ...
        MapId mapid = itemstack1.m_323252_(DataComponents.f_315230_);
        boolean flag3 = false;
    ...
    private void renderMap(
        GuiGraphics pGuiGraphics, @Nullable MapId p_334395_, @Nullable MapItemSavedData pMapData, int pX, int pY, float pScale
    ) {
        if (p_334395_ != null && pMapData != null) {
            pGuiGraphics.pose().pushPose();
            pGuiGraphics.pose().translate((float)pX, (float)pY, 1.0F);
            pGuiGraphics.pose().scale(pScale, pScale, 1.0F);
            this.minecraft.gameRenderer.getMapRenderer().render(pGuiGraphics.pose(), pGuiGraphics.bufferSource(), p_334395_, pMapData, true, 15728880);
            pGuiGraphics.flush();
            pGuiGraphics.pose().popPose();
        }
    }

...
public record FluidPredicate(Optional<HolderSet<Fluid>> f_315771_, Optional<StatePropertiesPredicate> properties) {
    public static final Codec<FluidPredicate> CODEC = RecordCodecBuilder.create(
        p_325215_ -> p_325215_.group(
                    RegistryCodecs.homogeneousList(Registries.FLUID).optionalFieldOf("fluids").forGetter(FluidPredicate::f_315771_),
    ...
    public boolean matches(ServerLevel pLevel, BlockPos pPos) {
        if (!pLevel.isLoaded(pPos)) {
            ...
            return this.f_315771_.isPresent() && !fluidstate.is(this.f_315771_.get())
                ? false

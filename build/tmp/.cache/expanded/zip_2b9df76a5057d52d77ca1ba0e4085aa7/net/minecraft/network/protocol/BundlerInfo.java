...
public interface BundlerInfo {
    ...
    static <T extends PacketListener, P extends BundlePacket<? super T>> BundlerInfo createForPacket(
        final PacketType<P> p_329276_, final Function<Iterable<Packet<? super T>>, P> pBundler, final BundleDelimiterPacket<? super T> pPacket
    ) {
        return new BundlerInfo() {
            @Override
            public void unbundlePacket(Packet<?> p_265538_, Consumer<Packet<?>> p_265064_) {
                if (p_265538_.write() == p_329276_) {
                    P p = (P)p_265538_;
            ...
            public BundlerInfo.Bundler startPacketBundling(Packet<?> p_265749_) {
                return p_265749_ == pPacket ? new BundlerInfo.Bundler() {
                    private final List<Packet<? super T>> f_315577_ = new ArrayList<>();

                    @Nullable
                    @Override
                    public Packet<?> addPacket(Packet<?> p_336207_) {
                        if (p_336207_ == pPacket) {
                            return pBundler.apply(this.f_315577_);
                        } else if (this.f_315577_.size() >= 4096) {
                            throw new IllegalStateException("Too many packets in a bundle");
                        } else {
                            this.f_315577_.add((Packet<? super T>)p_336207_);
                            return null;
                        }
                    }
                } : null;

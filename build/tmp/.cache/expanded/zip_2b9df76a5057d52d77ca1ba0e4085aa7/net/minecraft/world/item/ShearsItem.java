...
public class ShearsItem extends Item {
    ...
    public static Tool m_324982_() {
        return new Tool(
            List.of(
                Tool.Rule.m_321859_(List.of(Blocks.COBWEB), 15.0F),
                Tool.Rule.m_319062_(BlockTags.LEAVES, 15.0F),
                Tool.Rule.m_319062_(BlockTags.WOOL, 5.0F),
                Tool.Rule.m_322924_(List.of(Blocks.VINE, Blocks.GLOW_LICHEN), 2.0F)
            ),
            1.0F,
            1
        );
    ...
    public InteractionResult useOn(UseOnContext pContext) {
        ...
        if (blockstate.getBlock() instanceof GrowingPlantHeadBlock growingplantheadblock && !growingplantheadblock.isMaxAge(blockstate)) {
            ...
            level.m_322719_(GameEvent.BLOCK_CHANGE, blockpos, GameEvent.Context.of(pContext.getPlayer(), blockstate1));
            if (player != null) {
                itemstack.hurtAndBreak(1, player, LivingEntity.m_322775_(pContext.getHand()));
            }
    ...
    public InteractionResult interactLivingEntity(ItemStack stack, Player playerIn, LivingEntity entity, net.minecraft.world.InteractionHand hand) {
        if (entity instanceof net.minecraftforge.common.IForgeShearable target) {
            ...
            if (target.isShearable(stack, entity.level(), pos)) {
                var drops = target.onSheared(playerIn, stack, entity.level(), pos, stack.m_319737_().m_320299_(net.minecraft.world.item.enchantment.Enchantments.f_316753_));
                var rand = new java.util.Random();
                drops.forEach(d -> {
                    var ent = entity.spawnAtLocation(d, 1.0F);
                    ent.setDeltaMovement(ent.getDeltaMovement().add((double)((rand.nextFloat() - rand.nextFloat()) * 0.1F), (double)(rand.nextFloat() * 0.05F), (double)((rand.nextFloat() - rand.nextFloat()) * 0.1F)));
                });
                stack.hurtAndBreak(1, playerIn, LivingEntity.m_322775_(hand));
            }
            return InteractionResult.SUCCESS;
        }

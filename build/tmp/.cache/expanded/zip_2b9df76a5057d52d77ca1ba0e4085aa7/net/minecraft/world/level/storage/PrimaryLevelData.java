...
public class PrimaryLevelData implements ServerLevelData, WorldData {
    ...
    public static final String f_303393_ = "LevelName";
    ...
    private PrimaryLevelData(
        ...
        Set<String> pRemoveFeatureFlags,
        ...
        this.f_315660_ = p_328735_;
        this.spawnAngle = pSpawnAngle;
        this.gameTime = pGameTime;
        this.dayTime = pDayTime;
        this.version = pPlayerDataVersion;
        this.clearWeatherTime = pXSpawn;
        this.rainTime = pYSpawn;
        this.raining = pRaining;
        this.thunderTime = pZSpawn;
        ...
        this.wanderingTraderSpawnDelay = pVersion;
        this.wanderingTraderSpawnChance = pClearWeatherTime;
    ...
    public static <T> PrimaryLevelData parse(
        Dynamic<T> pDynamic, LevelSettings pSettings, PrimaryLevelData.SpecialWorldProperty pSpecialWorldProperty, WorldOptions pWorldOptions, Lifecycle pWorldGenSettingsLifecycle
    ) {
        long i = pDynamic.get("Time").asLong(0L);
        return new PrimaryLevelData(
            pDynamic.get("Player").flatMap(CompoundTag.CODEC::parse).result().orElse(null),
            pDynamic.get("WasModded").asBoolean(false),
            new BlockPos(pDynamic.get("SpawnX").asInt(0), pDynamic.get("SpawnY").asInt(0), pDynamic.get("SpawnZ").asInt(0)),
            pDynamic.get("SpawnAngle").asFloat(0.0F),
            i,
            pDynamic.get("DayTime").asLong(i),
            LevelVersion.parse(pDynamic).levelDataVersion(),
            pDynamic.get("clearWeatherTime").asInt(0),
            pDynamic.get("rainTime").asInt(0),
            pDynamic.get("raining").asBoolean(false),
            pDynamic.get("thunderTime").asInt(0),
            pDynamic.get("thundering").asBoolean(false),
            pDynamic.get("initialized").asBoolean(true),
            pDynamic.get("DifficultyLocked").asBoolean(false),
            WorldBorder.Settings.read(pDynamic, WorldBorder.DEFAULT_SETTINGS),
            pDynamic.get("WanderingTraderSpawnDelay").asInt(0),
            pDynamic.get("WanderingTraderSpawnChance").asInt(0),
            pDynamic.get("WanderingTraderId").read(UUIDUtil.CODEC).result().orElse(null),
            pDynamic.get("ServerBrands")
                .asStream()
                .flatMap(p_327546_ -> p_327546_.asString().result().stream())
                .collect(Collectors.toCollection(Sets::newLinkedHashSet)),
            pDynamic.get("removed_features").asStream().flatMap(p_327544_ -> p_327544_.asString().result().stream()).collect(Collectors.toSet()),
            new TimerQueue<>(TimerCallbacks.SERVER_CALLBACKS, pDynamic.get("ScheduledEvents").asStream()),
            (CompoundTag)pDynamic.get("CustomBossEvents").orElseEmptyMap().getValue(),
            pDynamic.get("DragonFight").read(EndDragonFight.Data.CODEC).resultOrPartial(LOGGER::error).orElse(EndDragonFight.Data.DEFAULT),
            pSettings,
            pWorldOptions,
            pSpecialWorldProperty,
            pWorldGenSettingsLifecycle
        ).withConfirmedWarning(pWorldGenSettingsLifecycle != Lifecycle.stable() && pDynamic.get("confirmedExperimentalSettings").asBoolean(false));
    }
    ...
    private void setTagData(RegistryAccess pRegistry, CompoundTag pNbt, @Nullable CompoundTag pPlayerNBT) {
        ...
        DynamicOps<Tag> dynamicops = pRegistry.m_318927_(NbtOps.INSTANCE);
        ...
        pNbt.putInt("SpawnX", this.f_315660_.getX());
        pNbt.putInt("SpawnY", this.f_315660_.getY());
        pNbt.putInt("SpawnZ", this.f_315660_.getZ());
    ...
    public void setSpawn(BlockPos pSpawnPoint, float pAngle) {
        this.f_315660_ = pSpawnPoint.immutable();

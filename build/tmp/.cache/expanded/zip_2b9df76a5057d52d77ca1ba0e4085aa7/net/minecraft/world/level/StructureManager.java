...
public class StructureManager {
    ...
    public List<StructureStart> startsForStructure(ChunkPos pChunkPos, Predicate<Structure> pStructurePredicate) {
        Map<Structure, LongSet> map = this.level.getChunk(pChunkPos.x, pChunkPos.z, ChunkStatus.f_316571_).getAllReferences();
    ...
    public List<StructureStart> startsForStructure(SectionPos pSectionPos, Structure pStructure) {
        LongSet longset = this.level.getChunk(pSectionPos.x(), pSectionPos.z(), ChunkStatus.f_316571_).getReferencesForStructure(pStructure);
        Builder<StructureStart> builder = ImmutableList.builder();
    ...
    public void fillStartsForStructure(Structure pStructure, LongSet pStructureRefs, Consumer<StructureStart> pStartConsumer) {
        for (long i : pStructureRefs) {
            ...
            StructureStart structurestart = this.getStartForStructure(
                sectionpos, pStructure, this.level.getChunk(sectionpos.x(), sectionpos.z(), ChunkStatus.f_317114_)
            );
    ...
    public StructureStart getStructureWithPieceAt(BlockPos pPos, TagKey<Structure> pStructureTag) {
        return this.m_324772_(pPos, p_327244_ -> p_327244_.is(pStructureTag));
    }

    public StructureStart getStructureWithPieceAt(BlockPos pPos, HolderSet<Structure> p_335973_) {
        return this.m_324772_(pPos, p_335973_::contains);
    }

    public StructureStart m_324772_(BlockPos p_328568_, Predicate<Holder<Structure>> p_330204_) {
    ...
    public boolean hasAnyStructureAt(BlockPos pPos) {
        ...
        return this.level.getChunk(sectionpos.x(), sectionpos.z(), ChunkStatus.f_316571_).hasAnyStructureReferences();
    }

    public Map<Structure, LongSet> getAllStructuresAt(BlockPos pPos) {
        SectionPos sectionpos = SectionPos.of(pPos);
        return this.level.getChunk(sectionpos.x(), sectionpos.z(), ChunkStatus.f_316571_).getAllReferences();
    }

    public StructureCheckResult checkStructurePresence(ChunkPos pChunkPos, Structure pStructure, StructurePlacement p_332450_, boolean pSkipKnownStructures) {
        return this.structureCheck.checkStart(pChunkPos, pStructure, p_332450_, pSkipKnownStructures);
    }

...
public interface LevelReader extends BlockAndTintGetter, CollisionGetter, SignalGetter, BiomeManager.NoiseBiomeSource {
    @Nullable
    ChunkAccess getChunk(int pX, int pZ, ChunkStatus p_333298_, boolean pNonnull);
    ...
    default Holder<Biome> getNoiseBiome(int pX, int pY, int pZ) {
        ChunkAccess chunkaccess = this.getChunk(QuartPos.toSection(pX), QuartPos.toSection(pZ), ChunkStatus.f_316460_, false);
    ...
    default ChunkAccess getChunk(int pChunkX, int pChunkZ) {
        return this.getChunk(pChunkX, pChunkZ, ChunkStatus.f_315432_, true);
    }

    default ChunkAccess getChunk(int pChunkX, int pChunkZ, ChunkStatus p_331751_) {
        return this.getChunk(pChunkX, pChunkZ, p_331751_, true);
    }

...
public class Drowned extends Zombie implements RangedAttackMob {
    ...
    public static AttributeSupplier.Builder m_320431_() {
        return Zombie.createAttributes().add(Attributes.f_316806_, 1.0);
    }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData) {
        ...
        if (this.getItemBySlot(EquipmentSlot.OFFHAND).isEmpty() && pLevel.getRandom().nextFloat() < 0.03F) {
            this.m_21035_(EquipmentSlot.OFFHAND, new ItemStack(Items.NAUTILUS_SHELL));
    ...
    public static boolean checkDrownedSpawnRules(
        ...
        if (!pServerLevel.getFluidState(pPos.below()).is(FluidTags.WATER) && !MobSpawnType.m_307588_(pMobSpawnType)) {
            return false;
        } else {
            Holder<Biome> holder = pServerLevel.getBiome(pPos);
            boolean flag = pServerLevel.getDifficulty() != Difficulty.PEACEFUL
                && (MobSpawnType.m_307154_(pMobSpawnType) || isDarkEnoughToSpawn(pServerLevel, pPos, pRandom))
                && (MobSpawnType.m_307588_(pMobSpawnType) || pServerLevel.getFluidState(pPos).is(FluidTags.WATER));
            if (flag && MobSpawnType.m_307588_(pMobSpawnType)) {
                return true;
            } else {
    ...
    protected void populateDefaultEquipmentSlots(RandomSource pRandom, DifficultyInstance pDifficulty) {
        if ((double)pRandom.nextFloat() > 0.9) {
            ...
            if (i < 10) {
                this.m_21035_(EquipmentSlot.MAINHAND, new ItemStack(Items.TRIDENT));
            } else {
                this.m_21035_(EquipmentSlot.MAINHAND, new ItemStack(Items.FISHING_ROD));
            }
        }
    }

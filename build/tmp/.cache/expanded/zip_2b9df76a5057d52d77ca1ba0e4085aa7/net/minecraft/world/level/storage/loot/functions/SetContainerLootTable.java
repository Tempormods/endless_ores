...
public class SetContainerLootTable extends LootItemConditionalFunction {
    public static final MapCodec<SetContainerLootTable> CODEC = RecordCodecBuilder.mapCodec(
        p_327600_ -> commonFields(p_327600_)
                .and(
                    p_327600_.group(
                        ResourceKey.codec(Registries.f_314309_).fieldOf("name").forGetter(p_327592_ -> p_327592_.name),
    ...
    public ItemStack run(ItemStack pStack, LootContext pContext) {
        if (pStack.isEmpty()) {
            ...
            pStack.m_322496_(DataComponents.f_314304_, new SeededContainerLoot(this.name, this.seed));
            return pStack;
        }
    }
    ...
    public void validate(ValidationContext pContext) {
        ...
        if (pContext.resolver().m_318772_(Registries.f_314309_, this.name).isEmpty()) {
            pContext.reportProblem("Missing loot table used for container: " + this.name.location());
        }
    }

    public static LootItemConditionalFunction.Builder<?> withLootTable(BlockEntityType<?> pType, ResourceKey<LootTable> p_334597_) {
        return simpleBuilder(p_327599_ -> new SetContainerLootTable(p_327599_, p_334597_, 0L, pType.builtInRegistryHolder()));
    }

    public static LootItemConditionalFunction.Builder<?> withLootTable(BlockEntityType<?> pType, ResourceKey<LootTable> p_332251_, long pSeed) {
        return simpleBuilder(p_327596_ -> new SetContainerLootTable(p_327596_, p_332251_, pSeed, pType.builtInRegistryHolder()));
    }
}
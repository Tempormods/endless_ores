...
public class SimpleContainer implements Container, StackedContentsCompatible {
    ...
    public boolean canAddItem(ItemStack pStack) {
        ...
        for (ItemStack itemstack : this.items) {
            if (itemstack.isEmpty() || ItemStack.m_322370_(itemstack, pStack) && itemstack.getCount() < itemstack.getMaxStackSize()) {
                flag = true;
                break;
            }
        }
    ...
    private void moveItemToOccupiedSlotsWithSameType(ItemStack pStack) {
        for (int i = 0; i < this.size; i++) {
            ItemStack itemstack = this.getItem(i);
            if (ItemStack.m_322370_(itemstack, pStack)) {
    ...
    public void fromTag(ListTag pContainerNbt, HolderLookup.Provider p_328200_) {
        this.clearContent();

        for (int i = 0; i < pContainerNbt.size(); i++) {
            ItemStack.m_323951_(p_328200_, pContainerNbt.getCompound(i)).ifPresent(this::addItem);
        }
    }
    ...
    public NonNullList<ItemStack> m_307332_() {
        return this.items;
    }
}
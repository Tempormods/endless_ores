...
public final class ProjectileUtil {
    private static final float f_303081_ = 0.3F;
    ...
    public static HitResult m_307639_(Entity p_311718_, Predicate<Entity> p_311003_, ClipContext.Block p_312093_) {
    ...
    private static HitResult getHitResult(
        Vec3 pStartVec, Entity pProjectile, Predicate<Entity> pFilter, Vec3 pEndVecOffset, Level pLevel, float p_310295_, ClipContext.Block p_310049_
    ) {
        Vec3 vec3 = pStartVec.add(pEndVecOffset);
        HitResult hitresult = pLevel.clip(new ClipContext(pStartVec, vec3, p_310049_, ClipContext.Fluid.NONE, pProjectile));
        if (hitresult.getType() != HitResult.Type.MISS) {
            vec3 = hitresult.getLocation();
        }

        HitResult hitresult1 = getEntityHitResult(pLevel, pProjectile, pStartVec, vec3, pProjectile.getBoundingBox().expandTowards(pEndVecOffset).inflate(1.0), pFilter, p_310295_);
        if (hitresult1 != null) {

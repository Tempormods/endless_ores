...
public class MobBucketItem extends BucketItem {
    private static final MapCodec<TropicalFish.Variant> f_315322_ = TropicalFish.Variant.f_315998_.fieldOf("BucketVariantTag");
    ...
    private void spawn(ServerLevel pServerLevel, ItemStack pBucketedMobStack, BlockPos pPos) {
        if (this.getFishType().spawn(pServerLevel, pBucketedMobStack, null, pPos, MobSpawnType.BUCKET, true, false) instanceof Bucketable bucketable) {
            CustomData customdata = pBucketedMobStack.m_322304_(DataComponents.f_315524_, CustomData.f_317060_);
            bucketable.loadFromBucketTag(customdata.m_323330_());
            bucketable.setFromBucket(true);
        }
    }

    @Override
    public void appendHoverText(ItemStack pStack, Item.TooltipContext p_329546_, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        if (this.getFishType() == EntityType.TROPICAL_FISH) {
            CustomData customdata = pStack.m_322304_(DataComponents.f_315524_, CustomData.f_317060_);
            if (customdata.m_318976_()) {
                return;
            }

            Optional<TropicalFish.Variant> optional = customdata.m_322213_(f_315322_).result();

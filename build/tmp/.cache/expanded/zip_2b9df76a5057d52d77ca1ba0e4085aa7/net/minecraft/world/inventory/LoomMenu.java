...
public class LoomMenu extends AbstractContainerMenu {
    ...
    private final HolderGetter<BannerPattern> f_316655_;
    ...
    public LoomMenu(int pContainerId, Inventory pPlayerInventory, final ContainerLevelAccess pAccess) {
        ...
        this.f_316655_ = pPlayerInventory.player.m_321891_().lookupOrThrow(Registries.BANNER_PATTERN);
    }
    ...
    private List<Holder<BannerPattern>> getSelectablePatterns(ItemStack pStack) {
        if (pStack.isEmpty()) {
            return this.f_316655_.m_255050_(BannerPatternTags.NO_ITEM_REQUIRED).map(ImmutableList::copyOf).orElse(ImmutableList.of());
        } else {
            return (List<Holder<BannerPattern>>)(pStack.getItem() instanceof BannerPatternItem bannerpatternitem
                ? this.f_316655_.m_255050_(bannerpatternitem.getBannerPattern()).map(ImmutableList::copyOf).orElse(ImmutableList.of())
                : List.of());
        }
    }
    ...
    public void slotsChanged(Container pInventory) {
        ...
        if (!itemstack.isEmpty() && !itemstack1.isEmpty()) {
            ...
            if (holder != null) {
                BannerPatternLayers bannerpatternlayers = itemstack.m_322304_(DataComponents.f_314522_, BannerPatternLayers.f_316086_);
                boolean flag1 = bannerpatternlayers.f_315710_().size() >= 6;
                if (flag1) {
                    this.selectedBannerPatternIndex.set(-1);
    ...
    private void setupResultSlot(Holder<BannerPattern> pPattern) {
        ...
        if (!itemstack.isEmpty() && !itemstack1.isEmpty()) {
            ...
            itemstack2.m_322591_(
                DataComponents.f_314522_,
                BannerPatternLayers.f_316086_,
                p_327092_ -> new BannerPatternLayers.Builder().m_322919_(p_327092_).m_318746_(pPattern, dyecolor).m_319179_()
            );
        }

...
public class MapItemSavedData extends SavedData {
    ...
    public static MapItemSavedData load(CompoundTag p_164808_, HolderLookup.Provider p_332149_) {
        ...
        RegistryOps<Tag> registryops = p_332149_.m_318927_(NbtOps.INSTANCE);

        for (MapBanner mapbanner : MapBanner.f_315914_
    ...
    @Override
    public CompoundTag save(CompoundTag pCompound, HolderLookup.Provider p_327825_) {
        ...
        RegistryOps<Tag> registryops = p_327825_.m_318927_(NbtOps.INSTANCE);
        pCompound.put("banners", MapBanner.f_315914_.encodeStart(registryops, List.copyOf(this.bannerMarkers.values())).getOrThrow());
    ...
    private static Predicate<ItemStack> m_322359_(ItemStack p_331084_) {
        MapId mapid = p_331084_.m_323252_(DataComponents.f_315230_);
        return p_327526_ -> p_327526_ == p_331084_
                ? true
                : p_327526_.is(p_331084_.getItem()) && Objects.equals(mapid, p_327526_.m_323252_(DataComponents.f_315230_));
    }
    ...
    public void tickCarriedBy(Player pPlayer, ItemStack pMapStack) {
        ...
        Predicate<ItemStack> predicate = m_322359_(pMapStack);
        if (!pPlayer.getInventory().m_321814_(predicate)) {
        ...
        for (int i = 0; i < this.carriedBy.size(); i++) {
            ...
            if (!mapitemsaveddata$holdingplayer1.player.isRemoved()
                && (mapitemsaveddata$holdingplayer1.player.getInventory().m_321814_(predicate) || pMapStack.isFramed())) {
                if (!pMapStack.isFramed() && mapitemsaveddata$holdingplayer1.player.level().dimension() == this.dimension && this.trackingPosition) {
                    this.addDecoration(
                        MapDecorationTypes.f_315151_,
        ...
        MapDecorations mapdecorations = pMapStack.m_322304_(DataComponents.f_316761_, MapDecorations.f_317068_);
        if (!this.decorations.keySet().containsAll(mapdecorations.f_315837_().keySet())) {
            mapdecorations.f_315837_()
                .forEach(
                    (p_327528_, p_327529_) -> {
    ...
    private void removeDecoration(String pIdentifier) {
        ...
        if (mapdecoration != null && mapdecoration.type().value().f_314463_()) {
    ...
    public static void addTargetDecoration(ItemStack pMap, BlockPos pTarget, String pDecorationName, Holder<MapDecorationType> p_335418_) {
        MapDecorations.Entry mapdecorations$entry = new MapDecorations.Entry(p_335418_, (double)pTarget.getX(), (double)pTarget.getZ(), 180.0F);
        pMap.m_322591_(DataComponents.f_316761_, MapDecorations.f_317068_, p_327532_ -> p_327532_.m_320603_(pDecorationName, mapdecorations$entry));
        if (p_335418_.value().m_324003_()) {
            pMap.m_322496_(DataComponents.f_315135_, new MapItemColor(p_335418_.value().f_315023_()));
        }
    }
    ...
    private void addDecoration(
        ...
        if (f >= -63.0F && f1 >= -63.0F && f <= 63.0F && f1 <= 63.0F) {
            ...
            if (!p_333390_.m_318604_(MapDecorationTypes.f_315151_)) {
            ...
            if (Math.abs(f) < 320.0F && Math.abs(f1) < 320.0F) {
                p_333390_ = MapDecorationTypes.f_314040_;
            } else {
                ...
                p_333390_ = MapDecorationTypes.f_314334_;
            }
        ...
        MapDecoration mapdecoration1 = new MapDecoration(p_333390_, b0, b1, b2, Optional.ofNullable(pName));
    ...
    @Nullable
    public Packet<?> getUpdatePacket(MapId p_328547_, Player pPlayer) {
    ...
    public boolean isExplorationMap() {
        for (MapDecoration mapdecoration : this.decorations.values()) {
            if (mapdecoration.type().value().f_314518_()) {
                return true;
            }
        }
    ...
    public void addClientSideDecorations(List<MapDecoration> pDecorations) {
        ...
        for (int i = 0; i < pDecorations.size(); i++) {
            ...
            if (mapdecoration.type().value().f_314463_()) {
                this.trackedDecorationCount++;
            }
        }
    }
    ...
    public static record MapPatch(int startX, int startY, int width, int height, byte[] mapColors) {
        public static final StreamCodec<ByteBuf, Optional<MapItemSavedData.MapPatch>> f_315636_ = StreamCodec.m_320617_(
            MapItemSavedData.MapPatch::m_319573_, MapItemSavedData.MapPatch::m_322032_
        );

        private static void m_319573_(ByteBuf p_334846_, Optional<MapItemSavedData.MapPatch> p_333957_) {
            if (p_333957_.isPresent()) {
                ...
                FriendlyByteBuf.m_319178_(p_334846_, mapitemsaveddata$mappatch.mapColors);
            } else {
                p_334846_.writeByte(0);
            }
        }

        private static Optional<MapItemSavedData.MapPatch> m_322032_(ByteBuf p_332582_) {
            ...
            if (i > 0) {
                ...
                byte[] abyte = FriendlyByteBuf.m_324528_(p_332582_);

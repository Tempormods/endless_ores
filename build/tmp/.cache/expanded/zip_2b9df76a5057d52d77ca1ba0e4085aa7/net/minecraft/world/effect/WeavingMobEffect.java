...
class WeavingMobEffect extends MobEffect {
    private final ToIntFunction<RandomSource> f_314740_;

    protected WeavingMobEffect(MobEffectCategory p_331231_, int p_336179_, ToIntFunction<RandomSource> p_328620_) {
        super(p_331231_, p_336179_, ParticleTypes.f_315496_);
        this.f_314740_ = p_328620_;
    }

    @Override
    public void m_319157_(LivingEntity p_335117_, int p_333338_, Entity.RemovalReason p_328096_) {
        if (p_328096_ == Entity.RemovalReason.KILLED && (p_335117_ instanceof Player || p_335117_.level().getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING))) {
            this.m_324020_(p_335117_.level(), p_335117_.getRandom(), p_335117_.getOnPos());
        }
    }

    private void m_324020_(Level p_331288_, RandomSource p_332035_, BlockPos p_329542_) {
        Set<BlockPos> set = Sets.newHashSet();
        int i = this.f_314740_.applyAsInt(p_332035_);

...
public record ConditionReference(ResourceKey<LootItemCondition> name) implements LootItemCondition {
    ...
    public static final MapCodec<ConditionReference> CODEC = RecordCodecBuilder.mapCodec(
        p_327641_ -> p_327641_.group(ResourceKey.codec(Registries.f_315752_).fieldOf("name").forGetter(ConditionReference::name))
    ...
    public void validate(ValidationContext pContext) {
        if (pContext.hasVisitedElement(this.name)) {
            ...
            pContext.resolver()
                .m_318772_(Registries.f_315752_, this.name)
                .ifPresentOrElse(
    ...
    public boolean test(LootContext pContext) {
        LootItemCondition lootitemcondition = pContext.getResolver().m_318772_(Registries.f_315752_, this.name).map(Holder.Reference::value).orElse(null);

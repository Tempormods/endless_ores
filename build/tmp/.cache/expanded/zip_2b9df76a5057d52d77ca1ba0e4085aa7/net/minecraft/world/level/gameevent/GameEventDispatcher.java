...
public class GameEventDispatcher {
    ...
    public void post(Holder<GameEvent> p_335078_, Vec3 pPos, GameEvent.Context pContext) {
        ...
        GameEventListenerRegistry.ListenerVisitor gameeventlistenerregistry$listenervisitor = (p_327435_, p_327436_) -> {
            if (p_327435_.getDeliveryMode() == GameEventListener.DeliveryMode.BY_DISTANCE) {
                list.add(new GameEvent.ListenerInfo(p_335078_, pPos, pContext, p_327435_, p_327436_));
            } else {
                p_327435_.handleGameEvent(this.level, p_335078_, pContext, pPos);
            }
        };
        boolean flag = false;
            ...
            for (int i2 = l; i2 <= k1; i2++) {
                ...
                if (chunkaccess != null) {
                    for (int j2 = k; j2 <= j1; j2++) {
                        flag |= chunkaccess.getListenerRegistry(j2).visitInRangeListeners(p_335078_, pPos, pContext, gameeventlistenerregistry$listenervisitor);
                    }
                }
            }
        }
        ...
        if (flag) {
            DebugPackets.sendGameEventInfo(this.level, p_335078_, pPos);
        }
    }

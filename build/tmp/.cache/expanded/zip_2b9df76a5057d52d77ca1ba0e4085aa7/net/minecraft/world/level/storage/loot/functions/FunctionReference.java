...
public class FunctionReference extends LootItemConditionalFunction {
    ...
    public static final MapCodec<FunctionReference> CODEC = RecordCodecBuilder.mapCodec(
        p_327570_ -> commonFields(p_327570_)
                .and(ResourceKey.codec(Registries.f_316898_).fieldOf("name").forGetter(p_327571_ -> p_327571_.name))
    ...
    public void validate(ValidationContext pContext) {
        if (pContext.hasVisitedElement(this.name)) {
            ...
            pContext.resolver()
                .m_318772_(Registries.f_316898_, this.name)
                .ifPresentOrElse(
    ...
    protected ItemStack run(ItemStack pStack, LootContext pContext) {
        LootItemFunction lootitemfunction = pContext.getResolver().m_318772_(Registries.f_316898_, this.name).map(Holder::value).orElse(null);
